...
(()=>{
  ...
  $.ajax({
    type:"POST",
    url:"data/03-shopcart/listPro.php",
    data:{uid:uid},
            ...
            <div class="imfor">
              ...
              <div class="pudc">
                <div class="pudc_information" id="${l.iid}">
                  <a href="product_details.html?lid=${l.lid}">
                  <img src="${l.sm}" class="lf"/></a>
  ...
  $(function () {
    ...
    $('#content_box_body').on('click', '.Each>span', function () {
      ...
      if ($(this).hasClass('normal')) {
        ...
        $.ajax({
          type: "POST",
          url: 'data/03-shopcart/updatePro.php',
...
// 单独
  ...
  function adddel() {
    ...
    $("#content_box_body").on('click', '.add', (function () {
      ...
      var id=$(".Each input").val();
      $.ajax({
        type: "POST",
        url: "data/03-shopcart/addPro.php",
        data: {lid: id, count: 1,uid:uid},
...
//去结算
  ...
  $('#go-buy').click(function () {
    ...
    if(totalPrice<=0){
      ...
      var iid=$(".Each>span.true").parents(".check").next().children(".pudc_information").attr("id");
      console.log(iid);
      //更新已经有isCheck为true
      $.ajax({
        type:"POST",
        url:"data/update_checked.php",
        data:{iid:iid,uid:uid,checked:1},
        success:function(data){
          console.log(data);
        },error:function(){
          console.log("网络故障");
